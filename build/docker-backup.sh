...
targetfile=$(basename "$target")
containername=$(docker container inspect "$id" --format '{{.Name}}'|cut -c2-)
cmd=$(containerwhich "$id" "pihole")
if [ -z "$cmd" ];then
log "error" "Cannot Backup Pi-hole Teleport in $containername, pihole missing"
else
log "trace" "Backing up Pi-hole Teleport in $containername"
if [[ $PARAM_SIMULATE == "" ]];then
mkdir -p "$targetdir"
docker exec "$id" sh -c 'pihole -a -t /tp.tar.gz ; cat /tp.tar.gz; rm /tp.tar.gz' >"$target"
fi
fi
}
...
if [ "$(containerwhich "$container" 'pihole')" ];then
backuppihole "$container" "$target"/"$containername"/"data/pihole-teleport.tar.gz"
fi
